 ...
 *
 *
 * to compile with OMP support: mex buildPhantom3D_C.c CFLAGS="\$CFLAGS -fopenmp -Wall -std=c99" LDFLAGS="\$LDFLAGS -fopenmp"
 * References:
 * [1] D. Kazantsev & V. Pickalov, "New iterative reconstruction methods for fan-beam tomography" IPSE, 2017
 * D. Kazantsev, 2016-17
 */

float BuildPhant3D_Func(float *A, int N, int Object, float C0, float x0, float y0, float z0, float a, float b, float c, float phi_rot);
        ...
        
{
    ...
    FILE *in_file = fopen("Phantom3DLibrary3.dat", "r"); // read only
    
    if (! in_file ) // equivalent to saying if ( in_file == NULL )
    {
        printf("oops, file can't be read\n");
        mexErrMsgTxt("File does not exist or cannot be read");
    }
    ...
    while(!feof(in_file))
    {
        if (fgets(tempbuff,100,in_file)) {
            ...
            /*check that we got the right model */
...
//              else {mexErrMsgTxt("The number of components is unknown");}
                ...
                for(i=0; i<Components; i++) {
                    ...
                    BuildPhant3D_Func(A, N, Object, C0, x0, y0, z0, a, b, c, phi_rot);
                }
                break;
            }
        }
