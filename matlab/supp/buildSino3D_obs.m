...
Tomorange_Z_Ar =linspace(Tomorange_Xmin, Tomorange_Xmax-H_x, N);
Zdel = (Tomorange_Z_Ar - z0(i)).^2;

C1 = -4 *log(2);
AnglesTot = length(angles);
AnglesRad = angles*(pi/180);

F = zeros(P,AnglesTot,N); % 3D sinogram dimensions
        ...
        for z = 1:N     
            ...
            for ll = 1:AnglesTot
                ...
                for j = 1:P
                    ...
                    AA6 = (AA3)*delta1;
                    if (AA6 < 1)
                        F(j,ll,z) = F(j,ll,z) + first_dr*(1. - AA6)^(1/2);  % sinogramm computing
                    end
                end
            end
            end
