...
objlist = modelfile2Dtolist(pathTP, model)
print (objlist)
phantom_2Da = TomoP2D.Object(N_size, objlist)
...
plt.title('{}''{}'.format('using dat',model))

a=fig.add_subplot(rows,cols,current+1)
plt.rcParams.update({'font.size': 21})
plt.imshow(phantom_2Da, vmin=0, vmax=1, cmap="BuPu")
plt.colorbar(ticks=[0, 0.5, 1], orientation='vertical')
plt.title('{}''{}'.format('using dicts',model))
